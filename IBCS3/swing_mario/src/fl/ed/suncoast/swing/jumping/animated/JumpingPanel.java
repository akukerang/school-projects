...
public class JumpingPanel extends JPanel
{
	...
	public void paintComponent(Graphics g)
	{
		...
		switch(this.p.getCurrentSpriteID())
		{
			...
			case 10:
			{
				g2d.drawImage(this.p.getExtraSprite(), null, (int)this.p.getX() + (this.p.getFacingRight()?-8:16), (int)this.p.getY() + 16);
				break;
			}
			default:
		}
				...
				if(!al_keysPressed.contains(this.s_key))
				{
					...
					if(!(this.s_key == "LEFT" && al_keysPressed.contains("RIGHT")) && !(this.s_key == "RIGHT" && al_keysPressed.contains("LEFT")))
					{
						...
						switch(this.s_key)
						{
							...
							case "LEFT": p.moveLeft(); p.setFacingRight(false); break;
							case "RIGHT": p.moveRight(); p.setFacingRight(true); break;
